...
export function createCharacter(input: CharacterInput): Character {
  ...
  getDb()
    .prepare(
      "INSERT INTO characters (id, name, appearance, description, voice, stranger_name, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    )
    .run(
      character.id,
      character.name,
...
export function updateCharacter(id: string, input: CharacterInput): Character | null {
  ...
  getDb()
    .prepare(
      "UPDATE characters SET name = ?, appearance = ?, description = ?, voice = ?, stranger_name = ?, updated_at = ? WHERE id = ?",
    )
    .run(
      updated.name,
      updated.appearance,
