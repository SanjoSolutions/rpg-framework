...
7. `pnpm run deploy` — `butler push` each zip to `sanjox/rpg-framework`, then push the branch and tag so GitHub Actions creates the GitHub release and uploads ZIP assets. Same caveat: bare `pnpm deploy` is a pnpm workspace builtin and will fail with `ERR_PNPM_CANNOT_DEPLOY`.
