#!/usr/bin/env node
// Build the production app: Next.js standalone build + per-target Node.js
// Single Executable Application (SEA) bundles. Run with: pnpm build [target ...]
//
// Steps:
//   1. `next build --webpack` produces .next/standalone/.
//   2. For each target, an official Node.js binary is downloaded, the SEA
//      blob is injected, and the standalone tree is staged at dist/<target>/app/.
//
...
//
// `pnpm run pack` is the next step — it zips each dist/<target>/ for distribution.
//
...
console.log(`\nDone. Per-target trees in ${distDir}/<target>/. Run \`pnpm run pack\` to create distributable ZIPs.`)
